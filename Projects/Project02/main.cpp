...
void output(vector<Sigma> essentail, vector<Sigma> sigma, vector<vector<int >> sop, string dot_file) {
    ...
    if (sop.size() == 0) {
        ofs << ".p " << (essentail.size() ) << "\n";
    }
    else { ofs << ".p " << (essentail.size() + sop[0].size()) << "\n"; }
        ...
        for (int k = 0; k < bf.size(); ++k) {
            for (int j = 0; j < n; ++j) {
                ...
                if (bf[k][j] == 2) {
                    if (j == n - 1) {
                        /*cout << "binary ";
                        for (int l = binary[i].size() - 1; l >= 0; --l) {
                            cout << binary[i][l];
                        }cout << " pass in \n";
                        cout << "BF ";
                        for (int l = 0; l < bf[k].size(); ++k) {
                            cout << bf[k][l];
                        }cout << "\n";*/
